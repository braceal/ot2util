...
class ProtoPiler: 
    ...
    def _parse_config(self,) -> None: 
        ...
        Args:
            config_yaml (dict): parsed yaml file 
    ...
    def yaml_to_protocol(self, out_file: PathLike) -> None: 
        ...
        for mount, name in self.mount_to_pipette.items(): 
            ...
            pipette_command = pipette_command.replace('#tip_racks#', f','.join(valid_tipracks))
        ...
        for i, command_block in enumerate(self.commands.commands): 
            ...
            for (volume, src, dst) in self._process_instruction(command_block):
                ...
                aspirate_command = aspirate_command.replace(
                    "#src#", f'deck["{wellplate_location}"]["{src}"]')
                ...
                dispense_command = dispense_command.replace(
                    "#dst#", f'deck["{wellplate_location}"]["{dst}"]')
    ...
    def _process_instruction(self, command_block: Command) -> List[str]:
        if (type(command_block.volume) is int and
            type(command_block.source) is str and 
            type(command_block.destination) is str): 
