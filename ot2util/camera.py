...
def get_color(img, center_br, center_origin, diameter_x, diameter_y, coordinate):
    H = []
    S = []
    V = []
...
def find_draw_fiducial(img):
    ...
    cameraMatrix = np.array([[ 600,    0, img.shape[0]/2],
                             [    0, 600, img.shape[1]/2],
                             [    0,    0,              1]])
    ...
    final_tl = np.array([0, 0])
    final_br = np.array([0, 0])
...
def initialize_camera(camera_id = 2):
    global cap
    cap = cv2.VideoCapture(camera_id)
    cap.set(3, 1920)
    cap.set(4, 1280)
    cap.set(5, 30)  #Set frame rate to 30 fps
...
def color_recognize(workdir, coordinate = np.array([5, 5]), experiment_id = 0):
    global cap
    ret, frame = cap.read()
    frame1, center_br, center_origin, diameter_x, diameter_y = find_draw_fiducial(frame)
    frame2, test_color, hsv_avg = get_color(frame, center_br, center_origin, diameter_x, diameter_y, coordinate)
...
def convert_coordinate(coordinate_org = 'A1'):
